...
class ChartDataService {
  ...
  Future<Map<String, List<ChartDataPoint>>> fetchChartData() async {
    ...
    for (var doc in journals.docs) {
      ...
      if (pre is num && post is num) {
        ...
        if (ts != null) {
          ...
          final dKey = '${date.year}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}';
          dailyAnxietyMap.putIfAbsent(dKey, () => {'sum': 0.0, 'count': 0, 'ts': midnight.millisecondsSinceEpoch.toDouble()});
          dailyAnxietyMap[dKey]!['sum'] += avg;
    ...
    for (var key in sortedKeys) {
      ...
      if (map['count'] > 0) {
        dailyAnxietyPoints.add(ChartDataPoint(map['ts'], map['sum'] / map['count']));
      }
    }
    dailyAnxietyPoints.sort((a, b) => a.x.compareTo(b.x)); // Guarantee clean chronological line
    ...
    for (var doc in moods.docs) {
      ...
      if (key.length == 10) {
        ...
        if (d != null) {
           ...
           if (score is num) {
             dailyMoodPoints.add(ChartDataPoint(midnight.millisecondsSinceEpoch.toDouble(), score.toDouble()));
           }
        }
      }
    }
